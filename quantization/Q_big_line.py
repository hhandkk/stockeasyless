...
def kline_application(df, N):
   ...
   （第 N+2 天）
    ...
    print("若证券股价连续 N 天下跌后出现大阳线，次日股价开盘上涨的次数（第N + 2天）:" + str(total_counts_1_sub) + ", 占比："
                                              + str(total_counts_1_sub / total_counts_1))
    print("证券股价连续 N 天上涨之后出现大阴线的总次数：" +
          str(total_counts_2))
    print("若证券股价连续 N 天上涨之后出现大阴线，次日股价开盘下跌的次数（第N + 2天）:" + str(total_counts_2_sub) + ", 占比："
                                              + str(total_counts_2_sub / total_counts_2))
    return (total_counts_1, total_counts_1_sub, total_counts_2,
            total_counts_2_sub)
